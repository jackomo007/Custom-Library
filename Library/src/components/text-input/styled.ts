...
import { ComponentSize, heights, width, sidePaddings } from "../../config/sizes";
...
export const StyledWrapper = styled.div<StyledWrapperProps>`
    position: relative;
    width: ${pr => pr.width ? pr.width : [pr.innerSize]+'px'};
...
export const StyledTextInput = styled.input<StyledTextInputProps>`
    ...
    color: #000;
    ${ pr => pr.disabled ? `
    background-color: ${palette.globalColors.neutrals.white};
    color: #5e5e5e;
    cursor: not-allowed;
